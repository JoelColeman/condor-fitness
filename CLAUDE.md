...
| Engine Fix — per-circuit rest (index.html) | Card dispatch in `renderCards()` now honors each exercise's own rest fields via `restOr(val, dflt)` (`Number.isFinite` gate — **0 is a real value meaning "no rest timer"**, it no longer falls through to the default). Applied to `ex.rest_between_rounds_sec` on circuit + finisher_circuit (finisher's round buttons previously hardcoded 0) and `ex.rest_sec` on strength/timed/superset. Bug predates Afterburner: every circuit got the global 90s `circuit_round` default. | ✅ Complete |
| Engine Content — protocol notes (program.json + Supabase) | New `scripts/add-engine-protocols.mjs` (idempotent, dry-run by default): sets full work/rest protocol text at the front of `notes` on all 22 Afterburner circuits/finisher_circuits, derived from block structure + athletic-cut-v1.md; restructures W2D1 EMOM stations into three explicit 60s minute slots. Applied to repo `program.json` (`--apply-file`) and the active Supabase `programs` row (`--apply-supabase`, programs table only, guarded: verifies the active row is Afterburner and deep-equals the pre-edit repo file via key-canonicalized compare). | ✅ Complete |
| Engine Feature — auto-cycling interval clock (index.html) | `buildCircuitCard` routes to new `buildIntervalClockCard` when every sub-exercise has `duration_sec > 0`. Big countdown cycles WORK (station name shown) → REST (`rest_between_rounds_sec`) → next round; `0` rest chains work phases directly (EMOM). Phase label, round counter, current + next exercise, Pause/Resume, "End early" (confirm, records rounds completed). Wall-clock `endAt` + 250ms tick with catch-up loop survives background throttling. Rounds recorded on `data-rounds-done`; `collectSessionExercises` prefers it over `.btn-round-done` counting, so the `{rounds_completed, total_rounds}` save shape is unchanged. Wake lock (re-acquired on visibilitychange) + `navigator.vibrate`, both guarded no-ops on iOS. Reps-based circuits (AMRAPs) keep tap-per-round. | ✅ Complete |
...
| 2026-09-01 | **Code edited Chat-owned `program.json`** (protocol text into 22 circuit `notes`, W2D1 EMOM stations restructured to 60s minute slots) and the live Supabase `programs` row. | Explicitly directed by Joel in the task brief (Job 2), same pattern as the 2026-04-27 complex restructure. Done via `scripts/add-engine-protocols.mjs` so the change is reviewable and idempotent, not hand-edited. | Chat-directed content edit; no Section A change. Chat should fold the protocol text into athletic-cut-v1.md's derived data if it regenerates program.json |
| 2026-09-01 | **W2D1 EMOM stations restructured beyond the notes-only brief**: `exercises_in_circuit` became three explicit `duration_sec: 60` minute slots (Bike Cals 40s hard / 15 KB swings / 30s ropes, rest of minute = rest). | The verify requirement "EMOM chains 60s stations with no rest gaps" is unsatisfiable without per-station durations — the interval clock only engages when every station has `duration_sec`. Spec's "EMOMs: minute assignments" wording supports minute-slot modelling. | No spec change — data modelling within Job 2's intent, flagged for Chat's awareness |
| 2026-09-01 | Interval clock auto-engages on the all-stations-timed gate, not an explicit program.json flag. | Keeps the feature data-driven with zero schema additions: any future circuit whose stations all carry `duration_sec` gets the clock for free; adding `reps`-only stations opts back into tap-per-round. | Spec update needed: Section A "Circuit Card" should describe the interval-clock variant and its gate (Chat-owned) |
...
| 2026-09-01 | Postgres jsonb does NOT preserve key order. Any "does the Supabase row equal the repo file" guard must canonicalize (recursive sorted-keys) before `JSON.stringify` compare — a raw stringify compare false-alarms "diverged" on identical data. | Engine protocols script |
| 2026-09-01 | Timer features verify in minutes, not real time: monkeypatch `Date.now` in the preview page (`__realNow` + mutable `__offset`), jump the offset past phase boundaries, and let the wall-clock catch-up loop process the transitions. Requires timers built on `Date.now`/`endAt` (not accumulated `setInterval` ticks) — which is also what makes them survive background throttling on a locked phone. | Interval clock verification |
| 2026-09-01 | `confirm()` blocks preview_eval — stub `window.confirm = () => true` before clicking anything confirm-guarded in the harness. | Interval clock verification |
