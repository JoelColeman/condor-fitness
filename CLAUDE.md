...
| Operation Afterburner — timeline update | Chat-authored `athlete.json` timeline edit applied mechanically to the repo file and the `athlete_profile.data` jsonb: Recomp closed out (`status: complete`, `show_phase_timeline: false`, `end_weight_lbs: 260`, one appended highlight — session count corrected to 31 of 32 against the logged data), a new `operation_afterburner` block inserted after it (`status: active`, `show_phase_timeline: true`, Aug 31 – Oct 25 2026, `color: red`), and `fall_2026` moved to start 2026-10-26 with a reworded summary. Supabase write touched only the `timeline` key of the single `athlete_profile` row; the other six keys were left exactly as the live row had them. | ✅ Complete |
...
| 2026-08-31 | **Recomp logged 31 of 32 scheduled sessions — W2D2 is missing.** The Chat-authored highlight read "Completed all 32 sessions"; corrected to "Completed 31 of 32 sessions" on Joel's approval. | Verified by diffing the 8×4 scheduled grid in the Recomp `programs.data` against non-bonus sessions: every week has sessions (so "finished every week" holds), but W2D2 was never logged. The pointer still reached W8D4 because it advances on each save, so nothing downstream was affected. | No spec change — content corrected at source |
...
- [x] ~~**Recomp highlight overstates the session count.**~~ Corrected 2026-08-31 to "Completed 31 of 32 sessions" in both the repo file and the `athlete_profile` jsonb. W2D2 was never logged.
