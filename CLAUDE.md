...
| Operation Afterburner — timeline update | Chat-authored `athlete.json` timeline edit applied mechanically to the repo file and the `athlete_profile.data` jsonb: Recomp closed out (`status: complete`, `show_phase_timeline: false`, `end_weight_lbs: 260`, one appended highlight), a new `operation_afterburner` block inserted after it (`status: active`, `show_phase_timeline: true`, Aug 31 – Oct 25 2026, `color: red`), and `fall_2026` moved to start 2026-10-26 with a reworded summary. Supabase write touched only the `timeline` key of the single `athlete_profile` row; the other six keys were left exactly as the live row had them. | ✅ Complete |
...
| 2026-08-31 | `athlete_profile.timeline` still has no Operation Afterburner block, and `show_phase_timeline` is set on the Operation Recomp entry. | athlete.json / `athlete_profile` is Chat-owned (Section D) — Code stops and flags rather than editing. Until it is updated, the dashboard's mini phase pills render Afterburner's phases inside the *Recomp* block's expand card, and there is no Afterburner block on the timeline. | **Resolved 2026-08-31** by the Chat-authored timeline update below. |
| 2026-08-31 | Recomp's `end` was written as **2026-08-22**, not the 2026-08-29 in the brief. | The brief said to prefer the last non-bonus Recomp session in Supabase if it differed. It does: the final logged session is `2026-08-22` W8D4 (`consolidate`). Nothing — bonus or otherwise — is logged after it. | No spec change — the brief's own conditional |
| 2026-08-31 | **Recomp logged 31 of 32 scheduled sessions — W2D2 is missing.** The appended highlight "Completed all 32 sessions … finished every week" was applied verbatim as Chat-authored content and is therefore overstated by one session. | Code does not rewrite Chat-owned prose, and the brief declared the content final. Verified by diffing the 8×4 scheduled grid in the Recomp `programs.data` against non-bonus sessions: every week has sessions (so "finished every week" holds), but W2D2 was never logged. The pointer still reached W8D4 because it advances on each save. | Chat-owned copy fix needed — logged as an Open Task |
| 2026-08-31 | Operation Afterburner uses `color: "red"`, the same swatch as Operation Spartan (`tl-color-red`, `#b82030`). | Chat-authored and final; both blocks render correctly and are far apart on the timeline. Noted only so a future palette pass knows the duplication is intentional, not a typo. | No spec change |
...
| 2026-08-31 | `athlete_profile` is a **single row** (`id, data, updated_at`) whose `data` jsonb holds the whole athlete.json. Updating it is one `update({data}).eq('id', row.id)` — read the live row first and spread it (`{...row.data, timeline}`) so app-written keys can't be clobbered. `body_weight` and `prs` also exist as their own tables; the copies inside `data` are what the dashboard actually renders. | Afterburner timeline update |
| 2026-08-31 | Comparing a local JSON file to a Supabase jsonb column with `JSON.stringify` always reports a difference — Postgres reorders object keys on storage. Use `assert.deepStrictEqual` to tell a real drift from a reordering. Bit both the `programs` and `athlete_profile` checks. | Afterburner |
...
- [x] ~~**athlete.json / `athlete_profile` needs an Operation Afterburner timeline block (Chat-owned).**~~ Done 2026-08-31 — Chat-authored update applied to both the repo file and the `athlete_profile` jsonb; phase pills now render inside the Afterburner card.
- [ ] **Recomp highlight overstates the session count (Chat-owned copy fix).** The appended highlight reads "Completed all 32 sessions"; the data shows **31 of 32**, with W2D2 never logged. "finished every week" is accurate — only the total is off. One-word fix, but Code will not edit Chat-owned prose.
