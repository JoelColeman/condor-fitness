...
| Fix: phase progress counter ignored logged sessions | Active phase now derives from training progress, not the calendar. New `getProgressWeekNum(lc)` returns the week of the next scheduled workout after the pointer (the pointer's own week once the program is complete; null when the pointer's week isn't in the program). `getActivePhase(phases, lc)` prefers it and falls back to `getCurrentWeekNum()` only when nothing is logged; a week outside the program clamps to the near end instead of always the last phase. Training Log `currentWeek` and `pillCls(phaseId)` read the same source, so the phase pills agree with the rows beneath them. | ✅ Complete |
| Operation Afterburner — Job 1 (new program) | `program.json` replaced with Operation Afterburner v1 (8 weeks × 4 days, anchor 2026-08-31). Phase ids `ab_foundation`/`ab_build`/`ab_deload`/`ab_peak` are namespaced so `inCurrentProgram()` cannot match any of the 56 Recomp/Spartan sessions. `calf_rules` and `rest_timer_defaults_sec` preserved; per-week `run_protocol`/`calf_gate` carried for the Day-2 dashboard sub-line; HR-zone targets live in exercise `notes`. Loaded into `public.programs` only (upsert on `key`, then flip `is_active`) — `backfill-supabase.ts` untouched. | ✅ Complete |
| Operation Afterburner — Job 2 (Program Complete panel) | `#pw-panel-complete` copy is now program-agnostic ("Program Complete" / "This training block is finished…"), and the panel gained a "+ Bonus session" escape hatch plus the standard Sign out · Dashboard footer. `renderMain()` routes to the panel when `nextWorkout()` returns null; both bonus and both sign-out buttons share one handler. `getRandomBonusAlt(prog, phaseId, type)` accepts a null `phaseId` (search the whole program) and falls back to repeating a scheduled day when no alternates exist. | ✅ Complete |
| Afterburner v1.1 — Job 1 (circuit rest fix) | Screen 3 circuit cards now use the circuit's own `rest_between_rounds_sec`; `0` is a real value (no rest timer at all, EMOM flow), and only a *missing* field falls back to `rest_timer_defaults_sec.circuit_round`. | ✅ Complete |
| Afterburner v1.1 — Job 2 (protocol text) | Circuit cards render the circuit's `notes` under the subtitle (full work/rest/rounds/modality — no more guessing protocol from the card title). Data audit: every Afterburner engine/circuit entry carries complete protocol text in `notes`. | ✅ Complete |
| Afterburner v1.1 — Job 3 (circuit clock) | Auto-cycling WORK/REST clock for duration-based circuits (every station has `duration_sec`, name not AMRAP): full-screen overlay with phase label, countdown, station name, round counter, pause/resume, end-early (logs completed rounds). `rest 0` chains rounds back-to-back. AMRAP/reps/distance circuits keep tap-per-round. Rounds tracked on `card.dataset.roundsCompleted`; session collection reads it, falling back to counting done-buttons for tap circuits. | ✅ Complete |
| Afterburner v1.1 — Job 4 (program data sync) | program.json + Supabase `programs` row updated to athletic-cut v1.1: W3 D1 row-erg 60/30 engine with all-out max-HR calibration final; W3 D4 incline power-walk block + 1-min max climb; W4 D4 / W5 D1 / W5 D4 converted to stepper/incline power-walk; W7 D1 conditional tread-vs-erg sprints; W7–8 tread items calf-gated; RPE 8–9 anchor on engines; D2 runs "ramp-stage pace only, never pushed for HR". Logged sessions untouched. | ✅ Complete |
| Afterburner v1.1 — Job 5 (anchor slide) | `meta.anchor_date` 2026-08-31 → 2026-09-14; phase dates foundation Sep 14–27 / build Sep 28–Oct 18 / deload Oct 19–25 / peak Oct 26–Nov 8; week date strings + `meta.race` end date (Nov 8) updated. Verified in stub harness: dashboard shows Week 1 for Sep 14–20, the three pre-anchor sessions (8/31, 9/12, 9/14) still render on their W1 rows, no negative weeks. | ✅ Complete |
...
| 2026-08-12 | **Active phase is now progress-derived, not calendar-derived.** `getActivePhase()` takes the resolved `lastCompleted` pointer and resolves the phase from the next scheduled workout's week; `getCurrentWeekNum()` (anchor + elapsed days) survives only as the no-sessions-logged fallback. Out-of-program weeks clamp to the near end rather than always the last phase, and a pointer whose week isn't in the program is treated as absent. | Operation Recomp is 8 weeks from a 2026-06-08 anchor, so calendar week ran to 9–10 while the athlete was in week 7. `getActivePhase()` fell through to the last phase (Consolidate) and the counter reported 0 of its 4 days, even though 10 Build sessions were logged and rendering in the Training Log. Rest days mean calendar week always drifts ahead of real progress; the pointer is the only thing that tracks it. | **Spec update needed (Section A):** "Session Count Logic" describes counting against `activePhase` without saying how the active phase is chosen — it should state that it follows the progress pointer, not the date. |
| 2026-08-31 | **Bonus sessions can now repeat a scheduled day, not just an alternate.** `getRandomBonusAlt()` previously drew only from `day.alternates`; it now falls back to the scheduled days of that type (`alt: null`) when the pool is empty. | Neither Recomp nor Afterburner defines a single `alternates` entry anywhere, so the bonus flow — including the new Program Complete escape hatch — would have dead-ended on "no alternates found", which is the exact failure the escape hatch exists to prevent. `alt: null` is the same shape "Start this workout" already passes, so `bonus: true` and the no-pointer-advance behaviour are unchanged. | Spec update needed: Section A "Bonus Session" says "Loads random alternate of that type from current phase" — it should note the scheduled-day fallback and that from the Program Complete panel the search spans the whole program, not one phase. |
| 2026-08-31 | **Circuit cards now render their `notes`.** `buildCircuitCard()` dropped `note`/`notes` entirely; only `buildFinisherCircuitCard()` rendered them. Added a `card-note` line mirroring the existing Strength/Run/Cardio pattern. | Afterburner puts HR-zone targets ("orange = 155–168 bpm, target 8 splat min") in exercise notes, and several of those exercises are circuits — the target would have been invisible at the gym. Also surfaces previously-hidden Recomp circuit notes. | No spec change — Section A already specifies notes on cards; this was a gap in coverage |
| 2026-08-31 | Continuous runs/treadmill blocks are modelled as `type: 'cardio'` + `duration_min`, not `type: 'run'`. | `buildRunCard()` defaults `intervals` to 8 when absent, so a continuous run would render as a fake 8-round interval session with checkboxes. `type: 'run'` is reserved for genuine run/walk intervals with explicit `intervals`/`run_sec`/`walk_sec`. | No spec change — card-type selection within existing schema |
| 2026-08-31 | Afterburner `day.type` values are D1 `strength`, D2 `run`, D3 `strength`, D4 `engine`. No day maps to `circuit`. | `day.type` is what the bonus-session selector filters on, so it must be one of the four selector types. The program has no circuit-shaped day, so the "Circuit" bonus button finds nothing and shows the existing not-found message. | No spec change — the four buttons are fixed in Section A; a program simply may not populate all four |
| 2026-08-31 | `athlete_profile.timeline` still has no Operation Afterburner block, and `show_phase_timeline` is set on the Operation Recomp entry. | athlete.json / `athlete_profile` is Chat-owned (Section D) — Code stops and flags rather than editing. Until it is updated, the dashboard's mini phase pills render Afterburner's phases inside the *Recomp* block's expand card, and there is no Afterburner block on the timeline. | Chat-owned content update needed — logged as an Open Task |
| 2026-09-15 | Code edited Chat-owned program.json (and its Supabase row). | Explicit direction in the task prompt — athletic-cut-v1.md (v1.1) named as source of truth for the sync. Same precedent as the 2026-04-27 complex restructure. | No — Chat-directed content edit |
| 2026-09-15 | `meta.version` bumped "1" → "1.1", which fires the program-change reset (`programKey` mismatch) once per device: `lastCompleted` resets to `{week:1, day:0}` and `workoutProgress` drops. | Benign by construction — cross-device sync immediately re-resolves the pointer to W1D3 from the session rows. Verified in the harness. | No — expected behavior of the reset path |
| 2026-09-15 | W1 D4 and W2 D4 tread blocks also converted to incline power-walk circuits, though not in the Job 4 bullet list. | The doc's standing conversion rule covers every tread push segment until Stage 3 clears, the doc's own W1/W2 D4 lines already read as power-walk, and W1 D4 is the next scheduled session — serving "push = 1 min jog" would contradict v1.1. | No — judgment call within the doc's standing rule, flagged here |
| 2026-09-15 | W6 deload engine did NOT get the RPE 8–9 anchor despite "all engine cards" in the job. | The doc holds W6 at the green–orange border ("do not chase splats"); RPE 8–9 would contradict the deload's intent. | No — flagged for Chat to confirm |
| 2026-09-15 | Circuit clock timing is wall-clock anchored (`endsAt` timestamp + catch-up loop), not tick-counted. | Mobile browsers throttle timers when the screen locks mid-interval; tick counting would silently stall the workout clock. | Spec update needed: Section A Screen 3 card list should describe the circuit clock (duration circuits) and the per-circuit rest rule |
...
| 2026-08-12 | Calendar-derived week and progress-derived week are different numbers and will keep diverging — rest days, travel, and missed sessions mean real progress always trails the anchor. Anything answering "where is the athlete now" (active phase, phase pills, session counts) must read the `lastCompleted` pointer. `getCurrentWeekNum()` is only meaningful before the first session of a program is logged. | Phase counter fix |
| 2026-08-12 | The owner-only RLS that makes the publishable key safe also blocks browser verification of the signed-in path. Workaround that does not weaken RLS: dump live tables with the service-role key in a Node script, then serve a temp copy of the page with `Condor.getSession` + the five `fetch*` functions stubbed from that dump. The real render code runs against real data with no auth. | Phase counter fix |
| 2026-08-31 | Google Drive mounts at `C:\Users\jchri\My Drive`, not `G:\`. Chat-authored program specs live at `C:\Users\jchri\My Drive\Claude Stuff\Condor Fitness\`. | Afterburner spec read |
| 2026-08-31 | Loading a new program is a three-statement write against `public.programs` only: upsert on `key` with `is_active:false`, `update({is_active:false}).neq('id', newId)`, then `update({is_active:true}).eq('id', newId)`. The old row is kept — only its flag flips — so a rollback is one UPDATE. `scripts/backfill-supabase.ts` must never be used for this; it wipes all six tables. | Afterburner load |
| 2026-08-31 | A new program's render path can be verified end-to-end without a browser or a login: run the real `app-shared.js` inside `vm.createContext()` with a stubbed `window`/`localStorage`/`document` and a service-role client, seed localStorage with the *previous* program's `programKey` and pointer, then replay `init()`'s reset block and the Screen 2 render. Proves the programKey reset fires and that no old sessions survive `inCurrentProgram()`. | Afterburner verification |
| 2026-08-31 | One-off `scripts/_*.mjs` helpers (generator, validator, loader, verifiers) are deleted once the task lands. `program.json` is the single committed artifact — a checked-in generator would compete with the Drive markdown for source-of-truth. | Afterburner |
| 2026-09-15 | Program round-trip tooling: `scripts/dump-program.ts` (read-only dump of the active `programs` row) and `scripts/push-program.ts` (repo program.json → Supabase; dry-run by default, requires `--apply`, refuses a program-name mismatch, writes a backup of the outgoing version to the OS temp dir first). | Editing program data now means editing program.json and pushing — no hand-editing JSON in the Supabase dashboard. | Program data workflow |
| 2026-09-15 | Stub-auth harness pattern (`.verify-tmp/`, untracked): copies of both pages + `stub.js` overriding `Condor.getSession`/all `fetch*`/`saveSession` with dumped live data, served by `serve.mjs` on :4173 (launch.json `verify-harness`). A harness-only `__ffwd` hook shifts the circuit clock's `endsAt` to fast-forward phases — which is also exactly how screen-lock catch-up is exercised. | App verification without OAuth | Verification workflow |
...
- [ ] **Verify the signed-in render path.** Partially closed 2026-08-12: the dashboard render path was driven against live Supabase data via a stubbed-auth harness (program, timeline, phase counter, Training Log logged rows, Skill Trends all render clean, no console errors). Still needs Joel's Google account for the parts a stub can't cover — real OAuth redirect, and a session save round-tripping from index.html.
- [ ] **Section A: "Session Count Logic" is now inaccurate** — it describes counting against the active phase without saying how that phase is picked. As of 2026-08-12 it follows the `lastCompleted` progress pointer, not the calendar (Chat-owned).
...
- [ ] **athlete.json / `athlete_profile` needs an Operation Afterburner timeline block (Chat-owned).** The timeline still ends at Operation Recomp, and `show_phase_timeline: true` sits on the Recomp entry — so the Afterburner phase pills currently render inside the Recomp expand card. Needs a new block (`show_phase_timeline: true`) with the flag removed from Recomp. Block dates are now Sep 14 – Nov 8 2026 after the 2026-09-15 anchor slide. Code will not edit this file.
- [ ] **Afterburner defines no `alternates`.** Every day has `alternates: []`, so "Swap to alternate" shows the empty-state message and bonus sessions fall back to repeating a scheduled day. Fine as shipped, but worth a Chat pass if real alternates are wanted.
- [ ] **Section A: Screen 3 needs the circuit clock + per-circuit rest rule** (Chat-owned). Circuit cards: notes render under the subtitle; rest timer uses the circuit's `rest_between_rounds_sec` (0 = none) with `rest_timer_defaults_sec` as fallback only; duration-based circuits run an auto-cycling WORK/REST clock (see 2026-09-15 Build Status rows).
- [ ] **Chat to confirm: W6 deload engine left without the RPE 8–9 anchor** — see the 2026-09-15 deviation row.
